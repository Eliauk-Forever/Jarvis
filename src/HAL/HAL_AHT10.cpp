...
bool HAL::AHT_measure(float *temp, float *humi) 
{
  	uint8_t data[6];
  	uint32_t d;
  	...
  	if (humi) 
	{
  	  	...
  	  	*humi = (float)d * 100 / 1048576;
  	}
  	return true;
}

float HAL::AHT_getTemperature() 
{
  	float result;
  	if (! AHT_measure(&result, NULL))
  	  	result = NAN;
  	return result;
}

float HAL::AHT_getHumidity() 
{
  	float result;
  	if (! AHT_measure(NULL, &result))
  	  	result = NAN;
  	return result;
}
